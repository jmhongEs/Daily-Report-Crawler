...
class CrawlData
{
    ...
    public function __construct()
    {
        ...
        '
        {
            ...
            "11":"postBodyData",
            "14":"https://finance.naver.com/sise/sise_index_day.naver?code=KOSPI",
            "15":"https://finance.naver.com/sise/sise_index_day.naver?code=KOSPI",
    ...
    function forCheckPath(){
        ...
        foreach ($dataURL as $key => $url) {
            if (!in_array($key, array(11,14, 15))) {
                $crawlDataArray[] = $this->crawler->checkPath($key, $dataPath[$key]);
                ...
                dump($extractedArray);
                foreach ($extractedArray as $extracted){
                        $extractedValues = array_values($extracted);
                        dump($extractedValues);
                        $crawlDataArray[] = $extractedValues[1];
                    };
            }
        }
        dump($crawlDataArray);
